/*
 * parsing/rule/impls/block/blocks/video.rs
 *
 * ftml - Library to parse Wikidot text
 * Copyright (C) 2019-2026 Wikijump Team
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with this program. If not, see <http://www.gnu.org/licenses/>.
 */

use super::prelude::*;
use crate::tree::{Alignment, FloatAlignment, VideoSource};

pub const BLOCK_VIDEO: BlockRule = BlockRule {
    name: "block-video",
    accepts_names: &["video"],
    accepts_star: false,
    accepts_score: false,
    accepts_newlines: false,
    parse_fn,
};

fn parse_fn<'r, 't>(
    parser: &mut Parser<'r, 't>,
    name: &'t str,
    flag_star: bool,
    flag_score: bool,
    in_head: bool,
) -> ParseResult<'r, 't, Elements<'t>> {
    debug!("Parsing video block (name {name}, in-head {in_head})");
    assert!(!flag_star, "Video doesn't allow star flag");
    assert!(!flag_score, "Video doesn't allow score flag");
    assert_block_name(&BLOCK_VIDEO, name);

    let (source, mut arguments) = parser.get_head_name_map(&BLOCK_VIDEO, in_head)?;
    let alignment = match arguments.get("align") {
        Some(value) => {
            let align = match value.as_ref() {
                "left" => Alignment::Left,
                "right" => Alignment::Right,
                "center" => Alignment::Center,
                _ => return Err(parser.make_err(ParseErrorKind::BlockMalformedArguments)),
            };

            Some(FloatAlignment {
                align,
                float: false,
            })
        }
        None => None,
    };

    let source = match VideoSource::parse(source) {
        Some(source) => source,
        None => return Err(parser.make_err(ParseErrorKind::BlockMalformedArguments)),
    };

    // The block source is already resolved from the block head.
    // Ignore any user-provided HTML src attribute to avoid conflicts.
    arguments.get("src");
    // TODO: html render settings to allow this?
    arguments.get("autoplay");

    // Build video
    let element = Element::Video {
        source,
        alignment,
        attributes: arguments.to_attribute_map(parser.settings()),
    };

    ok!(element)
}
