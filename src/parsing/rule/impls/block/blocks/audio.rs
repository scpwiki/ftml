/*
 * parsing/rule/impls/block/blocks/audio.rs
 *
 * ftml - Library to parse Wikidot text
 * Copyright (C) 2019-2026 Wikijump Team
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with this program. If not, see <http://www.gnu.org/licenses/>.
 */

use super::prelude::*;
use crate::tree::{Alignment, AudioSource, FloatAlignment};

pub const BLOCK_AUDIO: BlockRule = BlockRule {
    name: "block-audio",
    accepts_names: &["audio"],
    accepts_star: false,
    accepts_score: false,
    accepts_newlines: false,
    parse_fn,
};

fn parse_fn<'r, 't>(
    parser: &mut Parser<'r, 't>,
    name: &'t str,
    flag_star: bool,
    flag_score: bool,
    in_head: bool,
) -> ParseResult<'r, 't, Elements<'t>> {
    debug!("Parsing audio block (name {name}, in-head {in_head})");
    assert!(!flag_star, "Audio doesn't allow star flag");
    assert!(!flag_score, "Audio doesn't allow score flag");
    assert_block_name(&BLOCK_AUDIO, name);

    let (source, mut arguments) = parser.get_head_name_map(&BLOCK_AUDIO, in_head)?;
    let alignment = match arguments.get("align") {
        Some(value) => {
            let align = match value.as_ref() {
                "left" => Alignment::Left,
                "right" => Alignment::Right,
                "center" => Alignment::Center,
                _ => return Err(parser.make_err(ParseErrorKind::BlockMalformedArguments)),
            };

            Some(FloatAlignment {
                align,
                float: false,
            })
        }
        None => None,
    };

    // Parse the audio source based on format
    let source = match AudioSource::parse(source) {
        Some(source) => source,
        None => return Err(parser.make_err(ParseErrorKind::BlockMalformedArguments)),
    };

    // Build audio
    let element = Element::Audio {
        source,
        alignment,
        attributes: arguments.to_attribute_map(parser.settings()),
    };

    ok!(element)
}
